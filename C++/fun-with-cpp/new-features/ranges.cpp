...
void rangeSort() {
  ...
  std::ranges::sort(
      nums, [](const int &first, const int &last) { return first > last; });
...
void rangeTransformerFilter() {
  ...
  auto x = nums |
           std::ranges::views::filter([](const int &n) { return n % 2 == 0; }) |
...
int main() {
  ...
  printArrayElements(
      (std::ranges::views::iota(0, 10) | std::ranges::views::reverse),
